package org.lessons.org.lessons.lesson05.homeworks
...
должно быть зафиксировано. Лаборант приносит вам набор показателей, но по пути может что-нибудь потерять. Задача -
сообщить об ошибке в случае отсутствия показаний атмосферного давления.
 */
...
fun main() {
    ...
    coefficient = coefficient ?: baseCoefficient
    val resultIntensity = baseIntensity * coefficient

    // example 2
    val defaultCost: Double = 50.0
    var cost: Double? = 20.0 // но может быть null
    val deliveryCost = 5.0
    val insuranceCoefficient = 0.005
    // стоимость страховки
    val insuranceCost = (cost ?: defaultCost)
