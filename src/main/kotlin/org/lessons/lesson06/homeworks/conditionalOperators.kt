...
fun convertTemp(temp: Double, tempUnit: Char){
    ...
    */
    val degreeSign: Char = '°'
    val CelsiusDegreeSign: Char = 'C'
    val FahrenheitDegreeSign: Char = 'F'
    var outTemp: Double? = null
    if (tempUnit == CelsiusDegreeSign) {
        outTemp = temp * 1.8 + 32
        println("$outTemp$degreeSign$FahrenheitDegreeSign")
    } else if (tempUnit == FahrenheitDegreeSign) {
        outTemp = (temp - 32)/1.8
        println("$outTemp$degreeSign$CelsiusDegreeSign")
    } else {
        println("ERROR: wrong temp units")
    }
}

fun chooseClothes(airTemp: Double){
    /*
     ...
     */
    when {
        airTemp < -30.0 || airTemp > 35.0 -> println("Stay at home")
        airTemp < 10.0 -> println("куртка и шапка")
        airTemp <= 18.0 -> println("ветровка")
        else -> println("футболка и шорты")
    }
}

fun chooseMovieByAge(age: Int){
    /*
    ...
    */
    when {
        age <= 0 -> println("некорректный возраст")
        age > 0 && age < 10 -> println("детские")
        age >= 10 && age < 18 -> println("подростковые")
        else -> println("взрослые")
    }
